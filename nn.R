...
df_tr <-
  df_messed_collection_mod$df_test[[13]] %>% 
...
df_ts <-
  df_messed_collection_mod$df_train[[13]] %>% 
  select(contains("metric"), match) %>% 
  mutate_if(is.logical, as.integer) %>% 
  mutate_if(is.factor, as.integer) %>% 
  mutate_if(is.integer, as.double)
...
history <- 
  ...
  fit(
    ...
    verbose = 1,
    callbacks = list(callback_model_checkpoint("./best_nn.h5", verbose = T, save_best_only = T)
                     # callback_reduce_lr_on_plateau(patience=30, verbose = T)
